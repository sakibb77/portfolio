...
const Skills = () => {
  return (
    <div className="skills container mx-auto mt-40" id="skills">
      ...
      <div className="skills-wrapper grid grid-cols-2 mt-40 gap-20">
        <ul className="skills-left flex flex-col gap-10 ">
          {data
            .filter((_, i) => i < Math.floor(data.length / 2))
            .map((skill, i) => (
              <li key={skill.id}>
                <div className="skill-item flex gap-10 items-baseline">
                  <span className="skill-num text-white/50 text-7xl">
                    {String(skill.id).padStart(2, 0).padEnd(3, ".")}
                  </span>
                  <span className="skill-name text-[2rem]">{skill.title}</span>
                </div>
              </li>
            ))}
        </ul>
